...
class ChannelPruningEnv:
    ...
    def prune_kernel(self, op_idx, preserve_ratio, preserve_idx=None):
        ...
        # now assign
        op.weight.data = torch.from_numpy(rec_weight).to('cpu')
        ...
        if self.export_model:  # prune previous buffer ops
            ...
            for idx in range(prev_idx, op_idx):
                ...
                if type(m) == nn.Conv2d:  # depthwise
                    m.weight.data = torch.from_numpy(m.weight.data.cpu().numpy()[mask, :, :, :]).to('cpu')
                    if m.groups == m.in_channels:
                        m.groups = int(np.sum(mask))
                elif type(m) == nn.BatchNorm2d:
                    m.weight.data = torch.from_numpy(m.weight.data.cpu().numpy()[mask]).to('cpu')
                    m.bias.data = torch.from_numpy(m.bias.data.cpu().numpy()[mask]).to('cpu')
                    m.running_mean.data = torch.from_numpy(m.running_mean.data.cpu().numpy()[mask]).to('cpu')
                    m.running_var.data = torch.from_numpy(m.running_var.data.cpu().numpy()[mask]).to('cpu')
    ...
    def _extract_layer_information(self):
        ...
        with torch.no_grad():
            for i_b, (input, target) in enumerate(self.train_loader):  # use image from train set
                ...
                input_var = torch.autograd.Variable(input).to('cpu')
    ...
    def _validate(self, val_loader, model, verbose=False):
        ...
        with torch.no_grad():
            for i, (input, target) in enumerate(val_loader):
                ...
                input_var = torch.autograd.Variable(input).to('cpu')
                target_var = torch.autograd.Variable(target).to('cpu')
